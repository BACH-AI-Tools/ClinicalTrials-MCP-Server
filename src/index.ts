...
class ClinicalTrialsServer {
          ...
          inputSchema: {
            ...
            properties: {
              query: {
                type: "string",
                description:
                  "General search term (condition, intervention, etc.)",
              },
              condition: {
                type: "string",
          ...
          inputSchema: {
            ...
            properties: {
              ...
              exclusionKeywords: {
                type: "string",
                description:
                  "Keywords that should NOT appear in eligibility criteria",
              },
              inclusionKeywords: {
                type: "string",
                description:
                  "Keywords that should appear in eligibility criteria",
              },
              pageSize: {
                type: "number",
                description:
      ...
      try {
        switch (request.params.name) {
          ...
          case "search_by_eligibility_criteria":
            return await this.handleSearchByEligibilityCriteria(
              request.params.arguments
            );
  ...
  private formatDetailedStudy(study: Study) {
    return {
      ...
      locations:
        study.protocolSection.contactsLocationsModule?.locations?.slice(0, 10),
    };
  }
      ...
      if (args?.exclusionKeywords) {
        ...
        filteredStudies = studies.filter((study) => {
          const eligibilityCriteria =
            study.protocolSection.eligibilityModule?.eligibilityCriteria?.toLowerCase() ||
            "";
          return !exclusionWords.some((word: string) =>
            eligibilityCriteria.includes(word)
          );
        });
      }
      ...
      const results = filteredStudies.map((study) => ({
        ...
        eligibility: {
          ...
          healthyVolunteers:
            study.protocolSection.eligibilityModule?.healthyVolunteers || false,
          criteriaPreview:
            study.protocolSection.eligibilityModule?.eligibilityCriteria?.substring(
              0,
              200
            ) + "..." || "Not available",
        },
